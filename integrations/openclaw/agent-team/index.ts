...
function formatTeamContext(teamData: TeamData): string {
  ...
  lines.push("");
  lines.push("### 🔄 Task Execution Flow (Six Phases)");
  lines.push("");
  lines.push("**SEARCH → RECORD → ORIENT → DISPATCH → REVIEW → UPDATE**");
  lines.push("");
  lines.push("**IMPORTANT: All tasks must follow this flow without exception.**");
  lines.push("");
  lines.push("#### 1. SEARCH — Context Search");
  lines.push("- Search memory/ directory for historical records");
  lines.push("- Understand task background and relevant context");
  lines.push("");
  lines.push("#### 2. RECORD — Progress Logging");
  lines.push("- Create/update memory/YYYY-MM-DD.md");
  lines.push("- Log task start status");
  lines.push("");
  lines.push("#### 3. ORIENT — Orientation Phase (Core Leader Responsibility)");
  lines.push("- **Understand Requirements**: What does the user really want?");
  lines.push("- **Interview**: Clarify unclear requirements (max 5 questions / 2 rounds)");
  ...
  lines.push("");
  lines.push("#### 4. DISPATCH — Delegate/Execute");
  lines.push("- **Belongs to team member** → Delegate with full context");
  lines.push("- **Belongs to self** → Execute directly");
  lines.push("  - Simple task: Just do it");
  lines.push("  - Complex task: Write plan file → Execute → Checkpoint");
  lines.push("");
  lines.push("#### 5. REVIEW — Check Task Results");
  lines.push("- Review completed work against requirements");
  lines.push("- If task incomplete → Loop back to node 1 (SEARCH)");
  lines.push("");
  lines.push("#### 6. UPDATE — Update Progress Status");
  lines.push("- Update task status in memory/YYYY-MM-DD.md");
  lines.push("- Mark as completed or note remaining work");
  lines.push("");
  lines.push("### 📋 Complex Task Rules (Applies to Everyone)");
  lines.push("");
  lines.push("**Criteria**: >3 tool calls, involves multiple files, or needs >3 minutes.");
  lines.push("");
  lines.push("**Flow**:");
  lines.push("1. Write plan file — Create task-name-plan.md in work/");
  lines.push("2. After each step — Check off and update progress");
  lines.push("3. After each Phase — Create git checkpoint");
  lines.push("4. Upon completion — Delete plan file or move to archive/");
  lines.push("");
  lines.push("### 📝 Templates");
  lines.push("");
  lines.push("**Progress Log Template** (memory/YYYY-MM-DD.md):");
  lines.push("```");
  lines.push("## [Task Name] (HH:MM)");
  lines.push("- Executor: [Name/Team Member]");
  lines.push("- Goal: [One-line description]");
  lines.push("- Status: In Progress / Completed");
  lines.push("- Result: [Fill when complete]");
  lines.push("```");
  lines.push("");
  lines.push("**Plan File Template** (work/task-name-plan.md):");
  lines.push("```");
  lines.push("# [Task Name] Plan");
  lines.push("");
  lines.push("## Goal");
  lines.push("[What we want to achieve]");
  lines.push("");
  ...
  lines.push("");
  lines.push("## Risks");
  lines.push("- [Risk 1]: [Mitigation]");
  lines.push("");
  lines.push("## Execution");
  lines.push("");
  lines.push("### Phase 1: [Phase Name]");
  lines.push("- [ ] [Step 1]");
  lines.push("- [ ] [Step 2]");
  lines.push("");
  lines.push("## Checkpoints");
  lines.push("- [ ] Phase 1 complete: [commit hash]");
  lines.push("```");
  lines.push("");
