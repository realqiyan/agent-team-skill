...
function formatTeamContext(teamData: TeamData, currentAgentId: string): string {
  ...
  lines.push("");
  lines.push("**SEARCH → RECORD → ORIENT → DISPATCH → UPDATE → REVIEW**");
  lines.push("");
  ...
  lines.push("  ```");
  lines.push("");
  lines.push("### 5. UPDATE — Update Progress Status");
  lines.push("- Update progress in `memory/YYYY-MM-DD.md`");
  lines.push("- Can be executed multiple times during task");
  lines.push("");
  ...
  lines.push("- If task complete → Task finished");
  lines.push("");
